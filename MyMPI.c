    ...
    MPI_Comm comm)      /* IN - Communicator */
{
   ...
   *storage = (void *)my_malloc(id, (local_rows + temp_m) * *n * datum_size);
   *subs = (void **)my_malloc(id, (local_rows + temp_m) * PTR_SIZE);

   lptr = (void *)&(*subs[0]);
   rptr = (void *)*storage;
   for (i = 0; i < (local_rows + temp_m); i++)
   {
      *(lptr++) = (void *)rptr;
      rptr += *n * datum_size;
   }
   ...
   */
   ...
   for (int i = 0; i < local_rows + temp_m; i++)
   {
      for (int j = 0; j < *n; j++)
      {
   ...
   if (id == (p - 1))
   {
      for (i = 0; i < p - 1; i++)
      {
         // Reads Values from File and then sends it to the right proc
         x = fread(*storage, datum_size, (BLOCK_SIZE(i, p, *m)) * *n, infileptr);
         // This sends to i
         MPI_Send(*storage, BLOCK_SIZE(i, p, *m) * *n, dtype, i, DATA_MSG, comm);
      }
      // If it's its own proc, it stores in the second row to creat halo row
      for (int i = 0; i < 1; i++)
      {
         for (int j = 0; j < *n; j++)
         {
   ...
   else
   {
      if (id == 0)
      {
         // If it's the first process, then it will store in the first row, and the halo row goes after it.
         MPI_Recv(*storage, local_rows * *n, dtype, p - 1, DATA_MSG, comm, &status);
      }
      else
      {
         // If it's a proc that's in between the first and last proc, it stores in second row, in between two halo rows.
   ...
   if (!id)
   {
      ...
      if (p > 1)
      {
         ...
         for (i = 1; i < p; i++)
         {
            ...
            fwrite(bstorage, BLOCK_SIZE(i, p, m) * n, sizeof(dtype), fp);
         }
         free(b);
         free(bstorage);
         fclose(fp);
